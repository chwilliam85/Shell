...
hive -S -v -e "
CREATE TABLE IF NOT EXISTS cm_ods_safe.${db_table} AS SELECT uid,person_card,realname,mobile,status,lasttime FROM cm_ods_mask.${db_table}_delta WHERE 1=2;

ALTER TABLE cm_ods_safe.${db_table} SET FILEFORMAT ORCFile;

INSERT OVERWRITE TABLE cm_ods_safe.${db_table} SELECT uid,case when (length(person_card) > 0) then sha2(person_card,256) else person_card end as person_card,case when (length(encode(realname,'UTF-8')) <= 9) then replace(realname,substr(realname,2,1),'*') when (length(encode(realname,'UTF-8')) <= 12) then replace(realname,substr(realname,2,2),repeat('*',2)) when (length(encode(realname,'UTF-8')) <= 24) then replace(realname,substr(realname,2,length(realname)-2),repeat('*',length(realname))) else realname end as realname,case when (length(mobile) >= 4) then replace(mobile,substr(mobile,4,4),repeat('*',4)) else mobile end,status,lasttime FROM cm_ods_mask.${db_table}_delta;

CREATE TABLE IF NOT EXISTS cm_ods_mask.${db_table} LIKE cm_ods_safe.${db_table};

INSERT OVERWRITE TABLE cm_ods_mask.${db_table} SELECT uid,person_card,realname,mobile,status,lasttime FROM cm_ods_mask.${db_table}_delta;
