...
hive -S -v -e "
CREATE TABLE IF NOT EXISTS cm_ods_safe.${db_table} AS SELECT id,uid,mobile,role_name,shop_sub_id,type,status,role_id,openid,refund,auth_group_id,accept_msg,bind_time,multi_suid FROM cm_ods_mask.${db_table}_delta WHERE 1=2;
ALTER TABLE cm_ods_safe.${db_table} SET FILEFORMAT ORCFile;

INSERT OVERWRITE TABLE cm_ods_safe.${db_table} SELECT id,uid,case when (length(mobile) >= 4) then replace(mobile,substr(mobile,4,4),repeat('*',4)) else mobile end as mobile,role_name,shop_sub_id,type,status,role_id,openid,refund,auth_group_id,accept_msg,bind_time,multi_suid FROM cm_ods_mask.${db_table}_delta;

CREATE TABLE IF NOT EXISTS cm_ods_mask.${db_table} LIKE cm_ods_safe.${db_table};

INSERT OVERWRITE TABLE cm_ods_mask.${db_table} SELECT id,uid,mobile,role_name,shop_sub_id,type,status,role_id,openid,refund,auth_group_id,accept_msg,bind_time,multi_suid FROM cm_ods_mask.${db_table}_delta;
