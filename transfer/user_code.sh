...
hive -S -v -e "
CREATE TABLE IF NOT EXISTS cm_ods_safe.${db_table} AS SELECT code_id,code_name,uid,last_uid,aid,openid,is_default,permkey,qrid,table_name,cashiers,permkey_c,qrid_c,shop_sub_id FROM cm_ods_mask.${db_table}_delta WHERE 1=2;

ALTER TABLE cm_ods_safe.${db_table} SET FILEFORMAT ORCFile;

INSERT OVERWRITE TABLE cm_ods_safe.${db_table} SELECT code_id,code_name,uid,last_uid,aid,openid,is_default,permkey,qrid,table_name,cashiers,permkey_c,qrid_c,shop_sub_id FROM cm_ods_mask.${db_table}_delta;

CREATE TABLE IF NOT EXISTS cm_ods_mask.${db_table} LIKE cm_ods_safe.${db_table};

INSERT OVERWRITE TABLE cm_ods_mask.${db_table} SELECT code_id,code_name,uid,last_uid,aid,openid,is_default,permkey,qrid,table_name,cashiers,permkey_c,qrid_c,shop_sub_id FROM cm_ods_mask.${db_table}_delta;
