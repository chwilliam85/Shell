...
hive -S -v -e "
CREATE TABLE IF NOT EXISTS cm_ods_safe.${db_table} AS SELECT id,jf_openid,unionid,appid,subscribe_appid,openid,subscribe_id,pay_status,order_id FROM cm_ods_mask.${db_table}_delta WHERE 1=2;

ALTER TABLE cm_ods_safe.${db_table} SET FILEFORMAT ORCFile;

INSERT OVERWRITE TABLE cm_ods_safe.${db_table} SELECT id,jf_openid,unionid,appid,subscribe_appid,openid,subscribe_id,pay_status,order_id FROM cm_ods_mask.${db_table}_delta;

CREATE TABLE IF NOT EXISTS cm_ods_mask.${db_table} LIKE cm_ods_safe.${db_table};

INSERT OVERWRITE TABLE cm_ods_mask.${db_table} SELECT id,jf_openid,unionid,appid,subscribe_appid,openid,subscribe_id,pay_status,order_id FROM cm_ods_mask.${db_table}_delta;
